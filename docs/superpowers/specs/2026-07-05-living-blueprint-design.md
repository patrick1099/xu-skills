...
更新蓝图有实现泄漏风险——刚泡完实现细节的主 agent 容易把 how 写进只该有 what 的文档。用一个 fresh subagent 当**防火墙**（死命令「只准写可观察行为」），同时它也是**消化又大又吵的对话记录的一次性容器**，不污染主上下文。两个好处一起拿。

**三源并用**（互补，缺一都会偏）：

| 源 | 作用 | 为什么需要 |
|---|---|---|
| 现有 `BLUEPRINT.md` | 基底 | 覆盖更新的起点 |
| **本次对话 / transcript** | **意图源** | 「用户想要什么、为什么」最清楚的地方；代码常无法表达功能意图，且代码可能正是要被重构掉的旧实现——拿它当唯一真相会把旧形状焊回蓝图 |
| 当前代码 | 现实源 | 告诉 subagent「现在实际长啥样」，并帮判定哪些意图**真落地了** |

**subagent 三条死命令**：
1. **只写可观察行为，不写实现**（防实现泄漏）。
2. **只收最终拍板的意图，丢弃对话中途被否掉的废案**——这是读对话的头号坑；用代码「实际建了啥」辅助判定哪些是真要的。
3. **覆盖式对账更新**，并标出「意图 ≠ 当前代码」处（那是下次重构的缺口）。

**graceful**：transcript 在就读，不在（如新会话无相关历史）就退化到「当前代码 + 主 agent 递的一句话简报」。transcript 路径为会话专属、易失，仅在更新当下取用，**不把它作为持久依赖**。

**按规模缩放**（守低仪式）：功能大改/多处变动 → 派蒸馏 subagent；只加一个功能/记一条痛点的小改 → 主 agent 直接 inline 覆盖，别为一行字起一个 subagent。
...
- **内容模型完整**：`SKILL.md` 含 §3 五节骨架、§3 两条承重设计（行为契约 + 硬约束/自由声明）、§4 手动触发规则 + 更新方式（subagent 蒸馏/inline 缩放、三源、三条死命令）、§2 四条划界。
