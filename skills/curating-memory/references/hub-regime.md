...
**最容易搞错的一点**:`project:projx` 是**设备订阅条件**(本机 `projects` 含 projx 才纳入),
**不是**"只有 projx 工程的会话才看得见"——视图是**用户级全局视图**。所以在一台
`class=["work"] / projects=["projx"]` 的机器上,给记忆打 `project:projx` 或 `class:work`
