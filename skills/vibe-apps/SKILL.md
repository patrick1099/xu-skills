...
**REQUIRED SUB-SKILL**：用 `true-north:living-blueprint` 建并维护 `CLAUDE.md` 规约段 + `docs/BLUEPRINT.md` + `docs/踩坑.md`（模式 C、活文档、brainstorming 接缝都在那里）。vibe-apps **必用**——让 AI 照意图重构而非逆向猜代码。

## 脚手架（建目录）

```
mytool/
├── CLAUDE.md              # living-blueprint 规约段 + 下方 vibe-apps 架构约束段
├── docs/{BLUEPRINT.md,踩坑.md}   # ← 由 living-blueprint 建
...
**CLAUDE.md 追加 vibe-apps 架构约束段**（在 living-blueprint 规约段之外）：
```markdown
## 架构约束（vibe-apps）
